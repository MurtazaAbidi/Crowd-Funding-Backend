...
const getCampaignDetails = async (id) => {
  ...
  const rejected = await sqlConn.connection.query(`select * from rejected_campaign where campaign_id=${id}`)

  console.log({milestones:MileStonesList.rows, investors: InvestorsList.rows, comments: Comments.rows})
  return {milestones:MileStonesList.rows, investors: InvestorsList.rows, comments: Comments.rows, rejected:rejected.rows}
};
