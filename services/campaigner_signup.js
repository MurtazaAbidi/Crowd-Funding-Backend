...
const campaignersignup = async (signupDetails) => {
  ...
  const result = await sqlConn.connection.query(`SELECT * FROM users WHERE email LIKE '${signupDetails.email}'`);
  ...
  const insert = await sqlConn.connection.query(`INSERT INTO users (name, email, password, phone_no, cnic, office_address) VALUES ('${signupDetails.name}', '${signupDetails.email}', '${password}', '${signupDetails.phone}', '${signupDetails.cnic}', '${signupDetails.officeAddress}')`);
  console.log(insert)
